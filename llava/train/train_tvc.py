...
class ModelArguments:
    ...
    vision_tower_text_encoder: Optional[str] = field(default=None) # Shapiro
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments,
                 vit_text_tokenizer: transformers.PreTrainedTokenizer = None):
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        batch = dict(
            ...
            input_prompts = [instance['input_prompts'] for instance in instances]
        )
...
def train(attn_implementation=None):
    ...
    # 2. Actvating Gradient Checkpointing
    if training_args.gradient_checkpointing:
        if hasattr(model, "enable_input_require_grads"):
            model.enable_input_require_grads() #For the embedding to pass back the vision backbone
        else:
            def make_inputs_require_grad(module, input, output):
                output.requires_grad_(True)
            model.get_input_embeddings().register_forward_hook(make_inputs_require_grad)

    # 3. Getting LoRa of the language model
    if training_args.lora_enable:
        from peft import LoraConfig, get_peft_model
        llm_lora_config = LoraConfig(
            use_dora = training_args.llm_dora_enable,
    ...
    # 9. Loading and initializing the vision tower
    if model_args.vision_tower is not None:
        ...
        vision_model = model.get_model().vision_tower.vision_tower.vision_model
        
        # TODO: Check if the input will require grad or not.
        
        # if training_args.gradient_checkpointing:
        #     if hasattr(vision_model, "enable_input_require_grads"):
        #         vision_model.enable_input_require_grads() #For the embedding to pass back the vision backbone
        #     else:
        #         def make_inputs_require_grad(module, input, output):
        #             output.requires_grad_(True)
        #         vision_model.get_input_embeddings().register_forward_hook(make_inputs_require_grad)
                
        vit_lora_config = LoraConfig(
            dora_enable = training_args.vit_dora_enable,
            r=training_args.vit_lora_r,
            lora_alpha=training_args.vit_lora_alpha,
            target_modules=find_all_linear_names(vision_model),
            lora_dropout=training_args.vit_lora_dropout,
            bias=training_args.vit_lora_bias
        )
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args) # Shapiro : Here to Edit data and batching
    # 12. Intializing the trainer
